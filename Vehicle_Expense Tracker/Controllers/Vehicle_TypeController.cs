...
namespace Vehicle_Expense_Tracker.Controllers
{
    ...
    public class Vehicle_TypeController : ControllerBase
    {
        ...
        [HttpPost("Save")]
        public async Task<IActionResult> SaveVehicle_Type(Vehicle_Type vehicle)
        {
            try
            {
                var result = await _manageVehicle.SaveVehicle_Type(vehicle);
                if (result != null)
                {
                    return Ok(result);
                }
        ...
        [HttpPut("Update/{Id}")]
        public async Task<IActionResult> UpdateVehicle_Type(int Id, Vehicle_Type vehicle)
        {
            try
            {
                var result=await _manageVehicle.UpdateVehicle_Type(Id, vehicle);
                if (result.status == "OK")
                {
                    return Ok(result);
                }
